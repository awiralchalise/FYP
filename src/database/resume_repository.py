...
class ResumeRepository:
    ...
    def delete_resume(self, resume_id: str, user_id: str) -> bool:
        ...
        """
        try:
            # Best-effort cleanup for related analyses in environments where
            # database cascades or policies may not behave as expected.
            try:
                self.client.table("skill_gaps").delete().eq(
                    "resume_id", resume_id
                ).eq("user_id", user_id).execute()
            except Exception as cleanup_error:
                print(f"Warning deleting related skill gaps: {cleanup_error}")

            response = self.client.table("resumes").delete().eq(
                "id", resume_id
            ).eq("user_id", user_id).execute()
            ...
            verification = self.client.table("resumes").select("id").eq(
                "id", resume_id
            ).eq("user_id", user_id).execute()
            return not bool(getattr(verification, "data", None))
